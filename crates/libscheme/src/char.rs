...
pub fn init(it: &mut Interp) {
    ...
    it.register("char=?", Arity::Exact(2), |_it, a| {
        char_cmp(a, "char=?", false, |x, y| x == y)
    });
    it.register("char<?", Arity::Exact(2), |_it, a| {
        char_cmp(a, "char<?", false, |x, y| x < y)
    });
    ...
    it.register("char-ci=?", Arity::Exact(2), |_it, a| {
        char_cmp(a, "char-ci=?", true, |x, y| x == y)
    });
    it.register("char-ci<?", Arity::Exact(2), |_it, a| {
        char_cmp(a, "char-ci<?", true, |x, y| x < y)
    });
    ...
    it.register("char-alphabetic?", Arity::Exact(1), |_it, a| {
        Ok(Value::Bool(
            ch(&a[0], "char-alphabetic?")?.is_ascii_alphabetic(),
        ))
    });
    ...
    it.register("char-whitespace?", Arity::Exact(1), |_it, a| {
        Ok(Value::Bool(
            ch(&a[0], "char-whitespace?")?.is_ascii_whitespace(),
        ))
    });
    it.register("char-upper-case?", Arity::Exact(1), |_it, a| {
        Ok(Value::Bool(
            ch(&a[0], "char-upper-case?")?.is_ascii_uppercase(),
        ))
    });
    it.register("char-lower-case?", Arity::Exact(1), |_it, a| {
        Ok(Value::Bool(
            ch(&a[0], "char-lower-case?")?.is_ascii_lowercase(),
        ))
    });
    ...
    it.register("char-downcase", Arity::Exact(1), |_it, a| {
        Ok(Value::Char(
            ch(&a[0], "char-downcase")?.to_ascii_lowercase(),
        ))
    });
}
